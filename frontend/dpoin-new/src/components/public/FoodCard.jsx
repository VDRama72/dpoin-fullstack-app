...
// ✅ PERBAIKAN: Gunakan window.location.protocol untuk mengatasi masalah mixed content
const baseURL = `${window.location.protocol}//${import.meta.env.VITE_API_BASE_URL?.replace('http://', '').replace('https://', '').replace('/api', '') || 'localhost:4000'}`;
