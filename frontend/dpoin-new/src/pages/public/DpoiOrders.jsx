// ✅ FILE: src/pages/public/DpoiOrders.jsx (Revisi Lengkap & Sempurna)

import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
// ✅ PERBAIKAN: Gunakan instance `api` yang sudah terkonfigurasi
...
    console.log("DpoiOrders: handleOpenAddProductModal called for storeKey:", storeKey);
    const storeData = groupedCart[storeKey];
    if (!storeData) {
        console.error("DpoiOrders: Store data not found for storeKey:", storeKey);
        return;
    }
    console.log("DpoiOrders: Store data for modal:", storeData);
...
          console.error("DpoiOrders: sellerId is missing for store:", storeData.storeName);
          setModalError('ID penjual tidak ditemukan untuk toko ini.');
          setModalLoading(false);
          return;
      }
      
      console.log(`DpoiOrders: Fetching products for sellerId: ${storeData.sellerId}`);
      // ✅ PERBAIKAN: Gunakan instance `api` dan ubah nama query parameter menjadi 'seller'
      const res = await api.get(`/products?seller=${storeData.sellerId}`); 
      console.log("DpoiOrders: Products API response:", res.data);
      
...
      setModalLoading(false);
      console.log("DpoiOrders: Available products for modal (after filtering existing cart):", availableProducts);

      if (availableProducts.length === 0) {
        console.log("DpoiOrders: No new products found for this store to add to cart.");
      }

    } catch (err) {
      console.error("DpoiOrders: Gagal memuat produk untuk modal:", err.response?.data || err.message || err);
...
  // ✅ PERBAIKAN: Gunakan `import.meta.env.VITE_API_BASE_URL` untuk URL gambar
